"""
Generate graspness labels for FULL point cloud (including floor/table points).

This is a variant of generate_graspness.py that does NOT apply workspace masking.
The resulting graspness labels are suitable for training models that will be
deployed on real-world point clouds without object bounding box information.

Key difference from generate_graspness.py:
- Original: mask = (depth_mask & workspace_mask)  <- filters to object bounding box
- This:     mask = depth_mask                     <- keeps all valid depth points

Output is saved to 'graspness_full/' instead of 'graspness/' so both can coexist.
...
Usage:
    ...
    # For test scenes (100-189):
    python dataset/generate_graspness_full.py \
        --dataset_root /path/to/graspnet \
        --camera_type realsense \
        --scene_start 100 --scene_end 190

Time estimate: ~2-4 hours for 100 scenes on a decent GPU.
"""
...
parser.add_argument('--scene_end',   type=int, default=100, help='exclusive; 100(training) or 190(full)')
...
if __name__ == '__main__':
    ...
    # Save to graspness_full/ instead of graspness/
    save_path_root = os.path.join(dataset_root, 'graspness_full')

    num_views, num_angles, num_depths = 300, 12, 4
    fric_coef_thresh = 0.8
    point_grasp_num = num_views * num_angles * num_depths

    print(f"Generating FULL graspness labels (no workspace masking)")
    print(f"Output directory: {save_path_root}")
    print(f"Scenes: {cfgs.scene_start} to {cfgs.scene_end - 1}")
    print(f"Annotations per scene: {cfgs.ann_start} to {cfgs.ann_end - 1}")
    print()
    ...
    for scene_id in tqdm(range(cfgs.scene_start, cfgs.scene_end), desc='Scenes', position=0):
        ...
        for ann_id in ann_iter:
            # Check if already computed (for resume capability)
            output_file = os.path.join(save_path, str(ann_id).zfill(4) + '.npy')
            ...
            # KEY DIFFERENCE: Only use depth_mask, NO workspace masking
            # This includes floor/table/background points
            depth_mask = (depth > 0)
            mask = depth_mask  # <-- No workspace filtering!
            
            cloud_masked = cloud[mask]

            # Load camera pose (still needed for transforming grasp points to camera frame)
            ...
            np.save(output_file, cloud_masked_graspness)
    
    print(f"\nDone! Graspness labels saved to: {save_path_root}")
